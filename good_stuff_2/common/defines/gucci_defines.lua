
NDefines.NCountry.SPECIAL_FORCES_CAP_MIN = 500 -- default 24

NDefines.NDiplomacy.TENSION_DECAY = 0.25 -- reports as 0.5, bit works properly -- default 0.1
NDefines.NDiplomacy.TENSION_SIZE_FACTOR = 0.8 -- default 1.0
NDefines.NDiplomacy.TENSION_TIME_SCALE_MIN = 0 -- default 0.25

NDefines.NGame.GAME_SPEED_SECONDS = { 0.25, 0.1875, 0.125, 0.0625, 0.0 } -- default { 2.0, 0.5, 0.2, 0.1, 0.0 }
-- TODO test on bad CPU for { 0.25, 0.180666667, 0.111333334, 0.042, 0.0 }
-- pri 0.2 na X mu lagi late game-a (intel)
-- pri 0.04 na X mu lagi ot vreme na vreme (intel)
-- pri 0.03 na X pochva da mu lagi (na intela)
NDefines.NGame.LAG_DAYS_FOR_LOWER_SPEED = 20 -- default 10
NDefines.NGame.LAG_DAYS_FOR_PAUSE = 50 -- default 25

NDefines.NMilitary.BASE_DIVISION_BRIGADE_GROUP_COST = 10 -- default 20
NDefines.NMilitary.BASE_DIVISION_BRIGADE_CHANGE_COST = 2 -- default 5
NDefines.NMilitary.BASE_DIVISION_SUPPORT_SLOT_COST = 5 -- default 10
NDefines.NMilitary.CORPS_COMMANDER_DIVISIONS_CAP = 300 -- default 24
NDefines.NMilitary.FIELD_MARSHAL_DIVISIONS_CAP = 400 -- default 24
NDefines.NMilitary.FIELD_MARSHAL_ARMIES_CAP = 11 -- default 5
NDefines.NMilitary.MAX_ARMY_EXPERIENCE = 6900 -- default 500
NDefines.NMilitary.MAX_NAVY_EXPERIENCE = 6900 -- default 500
NDefines.NMilitary.MAX_AIR_EXPERIENCE = 6900 -- default 500
