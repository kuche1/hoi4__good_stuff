-- TODO
--NDefines.NCountry.SPECIAL_FORCES_CAP_MIN = 1.0 -- default 0.05
--SPECIAL_FORCES_CAP_MIN = 0 -- default 24

NDefines.NDiplomacy.TENSION_DECAY = 0.4 -- for some fucking reason setting this to 0.4 sets the decay to 0.5 -- default 0.1
NDefines.NDiplomacy.TENSION_SIZE_FACTOR = 0.85 -- default 1.0
NDefines.NDiplomacy.TENSION_TIME_SCALE_MIN = 0 -- default 0.25

NDefines.NGame.LAG_DAYS_FOR_LOWER_SPEED = 30 -- default 10
NDefines.NGame.LAG_DAYS_FOR_PAUSE = 75 -- default 25

NDefines.NMilitary.MAX_ARMY_EXPERIENCE = 6900 -- default 500
NDefines.NMilitary.MAX_NAVY_EXPERIENCE = 6900 -- default 500
NDefines.NMilitary.MAX_AIR_EXPERIENCE = 6900 -- default 500
